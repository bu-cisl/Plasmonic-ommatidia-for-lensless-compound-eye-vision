function    p = interpolation(pleft,pright,pleft2,pright2,R2,mask,shift1,shift2,m)
    ...
    shift1_tmp = shift1;
    shift1 = ceil(shift1);
    d = size(p1(:,91-shift1:92+shift1));
    p1(:,200-shift1:201+shift1) = repmat((R2(:,200)+R2(:,200))./2,1,d(2));
    ...
    %p2(:,1:200) = 0.6*p2left(:,1:200)+0.4*pleft2(:,1:200);
    shift2 = -ceil(abs(shift2));
    mask_blendcent()
    p2l_tmp = mask_tmp1.*p2left;
    p2r_tmp = mask_tmp2.*p2right;
    p2 = p2l_tmp+p2r_tmp;
%     p2(:,1:200) = p2left(:,1:200);
%     p2(:,201:400) = p2right(:,201:400);
%     d = size(p2(:,91-s(n):91:92+s(n)));
%     p2(:,91-s(n):91:92+s(n)) = repmat(pmid2(:,91),1,d(2));
  %  p2(:,91)=R2_1(:,200);
